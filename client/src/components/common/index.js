...
export const Modal = ({ open, onClose, title, children, size = 'md' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative bg-white rounded-2xl shadow-2xl w-full ${sizeClass} animate-fade-in max-h-[90vh] flex flex-col`}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm animate-fade-in p-6">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 mx-auto ${danger ? 'bg-red-100' : 'bg-sage-100'}`}>
          <ExclamationTriangleIcon className={`w-6 h-6 ${danger ? 'text-red-500' : 'text-sage-600'}`} />
        </div>
        <h3 className="text-center font-display text-lg font-semibold text-gray-800 mb-2">{title}</h3>
